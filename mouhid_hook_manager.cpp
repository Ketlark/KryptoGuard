...
--*/
{
    ...
    *pRegistrationHandle = NULL;
    ...
    {
        LogDebug("Registration limit reached.");
    ...
    if (!g_MhkManager.HookActive)
    {
        ...
        if (!NT_SUCCESS(ntstatus))
        {
            LogDebug("MhkpHookMouHidDeviceObjects failed: 0x%X", ntstatus);
            goto exit;
        }
    }

    LogDebug("MHK callbacks registered. (RegistrationHandle = %p)", pEntry);
...
)
{
    ...
    LogDebug("PnP notification callback called.");
    ...
    if (MousePnpNotificationEventArrival == Event) {
        //LogDebug("Received mouse PnP notification. (Arrival)");
        NTSTATUS status = MhmEnableMouHidMonitor();
        if (!NT_SUCCESS(status)) {
            LogDebug("Failed to re-anable MouHid Monitor : 0x%X (Arrival)", status);
        }
    }
}
...
)
{
    ...
    *ppHookContext = NULL;
    ...
    LogDebug("Creating MouHid hook context.");

    cbConnectDataFieldOffset = MhdGetConnectDataFieldOffset();
    if (!cbConnectDataFieldOffset)
    {
        LogDebug("MhdGetConnectDataFieldOffset failed.");
    ...
    if (!NT_SUCCESS(ntstatus))
    {
        LogDebug("ObReferenceObjectByName failed: 0x%X", ntstatus);
        goto exit;
    }
    //
    ...
    if (!NT_SUCCESS(ntstatus))
    {
        LogDebug("IouEnumerateDeviceObjectList failed: 0x%X", ntstatus);
        goto exit;
    }
...
)
{
    ...
    LogDebug("Installing connect data hooks:");
...
)
{
    ...
    LogDebug("Hooking MouHid device objects.");

    ntstatus = MhkpCreateHookContext(pServiceCallbackHook, &pHookContext);
    if (!NT_SUCCESS(ntstatus))
    {
        LogDebug("MhkpCreateHookContext failed: 0x%X", ntstatus);
        goto exit;
    }
...
--*/
{
    ...
    {
        LogDebug("Unhandled class device object: %p", pDeviceObject);
        DEBUG_BREAK;
        goto exit;
    }
