...
--*/
{
    ...
    LogDebug("Enabling %s.", MODULE_TITLE);

    ExEnterCriticalRegionAndAcquireResourceExclusive(&g_MhmContext.Resource);

    if (g_MhmContext.RegistrationHandle)
    {
        LogDebug("%s is already enabled.", MODULE_TITLE);
    ...
    if (!NT_SUCCESS(ntstatus))
    {
        LogDebug("MhkRegisterCallbacks failed: 0x%X", ntstatus);
        goto exit;
    }

    LogDebug("%s enabled.", MODULE_TITLE);
...
MhmDisableMouHidMonitor()
{
    ...
    LogDebug("Disabling %s.", MODULE_TITLE);
    ...
    {
        LogDebug("%s is not enabled.", MODULE_TITLE);
        goto exit;
    }
    ...
    if (!NT_SUCCESS(ntstatus))
    {
        LogDebug("MhkUnregisterCallbacks failed: 0x%X", ntstatus);
        goto exit;
    }
    ...
    LogDebug("%s disabled.", MODULE_TITLE);
...
)
{
    ...
    //
    for (pInputPacket = pInputDataStart; pInputPacket < pInputDataEnd; ++pInputPacket) {
        PacketIndex = InterlockedIncrement64(&pCallbackContext->PacketIndex);
        HandleClickMouse(pInputPacket);

       // MclPrintInputPacket(PacketIndex, pServiceCallbackOriginal, pClassDeviceObject, pInputPacket);
    }

    //
    // Invoke the original service callback.
    //
    pServiceCallbackOriginal(pClassDeviceObject, pInputDataStart, pInputDataEnd, pnInputDataConsumed);
}
...
VOID
NTAPI
MhmpNotificationCallback(HANDLE RegistrationHandle, MOUSE_PNP_NOTIFICATION_EVENT Event, PVOID pContext) {
    UNREFERENCED_PARAMETER(pContext);

    ExEnterCriticalRegionAndAcquireResourceExclusive(&g_MhmContext.Resource);
    if (g_MhmContext.RegistrationHandle != RegistrationHandle) {
        LogDebug("Unexpected registration handle: %p", RegistrationHandle);
        goto exit;
    }
